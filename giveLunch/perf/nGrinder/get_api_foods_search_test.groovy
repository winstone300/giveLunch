...
class GiveLunchFoodSearchOnlyTest {
    ...
    static void beforeProcess() {
        ...
        runId = props.getProperty("runId", "food-search-${System.currentTimeMillis()}")

        testSearchFoodId = new GTest(1, "GET /api/foods/search")
    }

    @BeforeThread
    void beforeThread() {
        authRequest = new HTTPRequest()
        searchRequest = new HTTPRequest()
        testSearchFoodId.record(searchRequest)
        login()
    ...
    @Before
    void before() {
        setHeaders(searchRequest, "food-search")
    }
    ...
    void searchFoodIdScenario() {
        ...
        HTTPResponse resp = searchRequest.GET(urlFor("/api/foods/search?name=${urlEncode(query)}"))
